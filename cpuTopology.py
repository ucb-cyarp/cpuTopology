...
def printSMTCoresPerCore(topology):
    """
    Print the logical CPUs per physical CPU
    """

    coresToCPUs: Dict[int, List[int]] = {}
    ...
    for entry in topology:
        ...
        if core not in coresToCPUs:
            coresToCPUs[core] = []
        coresToCPUs[core].append(cpu)

    sortedCores = sorted(coresToCPUs.keys())
    
    print('Core | Logical CPUs')
    for core in sortedCores:
        print('{:>4d} |'.format(core), end='')
        sortedCPUs = sorted(coresToCPUs[core])
        for cpu in sortedCPUs:
            print(' {:>4d}'.format(cpu), end='')
        print('')